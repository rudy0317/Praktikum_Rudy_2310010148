...
<body>
  <div class="shell">
    <h1 class="title">
      ...
      <span>Form Edit Prodi</span>
      <span class="badge">Rudy • 2310010148</span>
    </h1>

    <div class="card">
      <form action="<?= site_url('prodi/perbaharui/'.$prodi->id_prodi) ?>" method="post" class="grid" autocomplete="off">
        <div class="row">
          <div>
            <label for="kode_prodi">Kode Program Studi</label>
            <input type="text" name="kode_prodi" id="kode_prodi" class="field" value="<?= html_escape($prodi->kode_prodi) ?>" required>
          </div>
          <div>
            <label for="nama_prodi">Nama Program Studi</label>
            <input type="text" name="nama_prodi" id="nama_prodi" class="field" value="<?= html_escape($prodi->nama_prodi) ?>" required>
          </div>
        </div>

        <div class="actions">
          <button type="submit" class="btn btn-primary">💾 Perbaharui</button>
          <a href="<?= site_url('prodi') ?>" class="btn btn-ghost">⬅ Kembali</a>
        </div>
      </form>
    </div>
  </div>
